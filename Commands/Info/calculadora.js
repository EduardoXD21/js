...
module.exports = {
    name: "calculadora",
    description: "Sirve para calcular 😐",
    /**
     * 
    ...
    run: async (client, message, args) => {
        ...
        col.on("collect", async (c) => {
            ...
            if (c.user.id !== message.author.id) return c.followUp(
                {
                    content: client.e + "  **❌**  solo <@!" + message.author.id + "> puede usar estos botones!",
                    ephemeral: true
                }
            )
        ...
        col.on("end", async () => {
            msg.edit({
                ...
                embeds: [emb.setDescription(`\`\`\`Se acabo el tiempo!\`\`\``).setTimestamp(Date.now())]
            })
        })
    }
}